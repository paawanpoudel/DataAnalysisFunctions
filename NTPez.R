...
#  suppressWarnings()
  ...
  # expression data
  exp.dataset<-read.delim(input.exp.filename,header=T,check.names=F); exp.dataset<-cbind(exp.dataset[,1],exp.dataset)
  ...
  exp.for.sample.names<-read.delim(input.exp.filename,header=F)  # read sample names
  sample.names<-as.vector(as.matrix(exp.for.sample.names[1,2:length(exp.for.sample.names[1,])]))
...
#  }
  ...
  if (GenePattern.output=="T"){
    ...
    # exp data
    write.table("#1.2",paste(output.name,"_sorted.dataset.txt",sep=""),quote=F,sep="\t",row.names=F,col.names=F)
    write.table(paste(num.features.extract,num.samples,sep="\t"),paste(output.name,"_sorted.dataset.txt",sep=""),quote=F,sep="\t",row.names=F,col.names=F,append=T)
    write.table(sorted.dataset,paste(output.name,"_sorted.dataset.txt",sep=""),quote=F,sep="\t",row.names=F,col.names=F,append=T)
